...
void StageRegistry::Update(){
	...
	if(stageEditor_->GetIsChangeStage()){
		DestroyMapData();
		mapData_= MapChipLoader::Load(kDirectoryPath_,stageEditor_->GetStageName());

		CreatesStageByMapData();
		ResetPlayer();
...
void StageRegistry::Register(const std::string& _fileName){
	...
	// mapの修正
	DestroyMapData();
	mapData_= MapChipLoader::Load(kDirectoryPath_,information.csvName);
	CreatesStageByMapData();
...
void StageRegistry::CreatesStageByMapData(){
	// stageの情報を登録
	DestroyStageData();
	stageData_.clear();
	maxSize_.x = static_cast<int>(mapData_[0].size());
	maxSize_.y = static_cast<int>(mapData_.size());
	...
	for(size_t row = 0; row < maxSize_.y; ++row){
		for(size_t col = 0; col < maxSize_.x; ++col){
			stageData_[row][col] = CreateBlock(mapData_[row][col]);
...
void StageRegistry::DestroyStageData(){
	// stageデータを削除
	for(size_t row = 0; row < maxSize_.y; ++row){
		for(size_t col = 0; col < maxSize_.x; ++col){
