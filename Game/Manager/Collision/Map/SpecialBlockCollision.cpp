...
#include "Game/Commands/Stage/ChangeBlockTypeSpecialToGhostBlock.h"
#include "Game/Commands/Stage/ChangeBlockTypeGhostBlockToSpecial.h"
...
void SpecialBlockCollision::ChangeBlock(){
	...
	for(const auto& row : data){
		for(const auto& col : row){
			// 特殊ブロック以外ならコンティニュー
			if(col == nullptr || !col->GetIsSpecialBlock() || col->GetType() != BlockType::GhostBlock){ continue; }
			// 特殊ブロックなら8方向に判定を取る
			...
			if(!isHit){
				if(data[sIndex.y][sIndex.x]->GetIsSpecialBlock()){
					// ゴーストブロックから特殊ブロックに戻す
					auto changeCommand = std::make_unique<ChangeBlockTypeGhostBlockToSpecial>(system_->GetStageRegi(),sIndex);
					ObjectCommandInvoker::GetInstance().PushCommand(std::move(changeCommand));
				}
			}
		}
	}
...
void SpecialBlockCollision::RecursionBlockChecker(const Vector2Int& _index){
	...
	for(auto& index : system_->GetNeighborOffsets()){
		...
		if(data[nextIndex.y][nextIndex.x]->GetType() == BlockType::SpecialBlock){
			// Command を積む
			auto changeCommand = std::make_unique<ChangeBlockTypeSpecialToGhostBlock>(system_->GetStageRegi(),nextIndex);
			ObjectCommandInvoker::GetInstance().PushCommand(std::move(changeCommand));
		}
