...
void ClearUIs::Init(){
	...
	// Clear_pot
	pot_ = Engine::GetCanvas2d()->AddSprite("pot.png","Pot","Sprite_Normal.json",1,true);
	pot_->ApplySaveData();
	AddChild(pot_);

	potion_ = Engine::GetCanvas2d()->AddSprite("potion.png","Potion","Sprite_Normal.json",1,true);
	potion_->ApplySaveData();
	AddChild(potion_);

	// Clear_curtain
	curtain_ = Engine::GetCanvas2d()->AddSprite("Clear_curtain.png","Curtain","Sprite_Normal.json",1,true);
	curtain_->ApplySaveData();
	AddChild(curtain_);
	...
	potion_->SetTranslate(Vector2{640.0f,360.0f});
